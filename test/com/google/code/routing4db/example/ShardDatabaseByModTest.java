...
@ContextConfiguration(locations = "classpath:shard-database-by-mod.xml")
public class ShardDatabaseByModTest extends TestCase {
    ...
    static {
        dataSources.add("dataSourceOne");
        dataSources.add("dataSourceTwo");
        dataSources.add("dataSourceThree");
        dataSources.add("dataSourceFour");
    }
    ...
    public void testInsert() {
        for (int i = 1; i <= 10; i++) {
            ...
            user.setName("User" + i);
            // 插入
            System.out.println("插入返回的值：" + userDao.insert(user));
    ...
    public void testTransaction() {
        for (int i = 10000; i <= 10080; i++) {
            ...
            RoutingHolder.setCurrentDataSourceKey(dataSources.get(i % dataSources.size())); // Master
